<p align="center"><img src="https://docs.celestia.world/res/logo/noa.webp" alt="noa" width="200" /></p>

<h1 align="center">Noa</h1>

<p align="center"><strong>ИИ-нативная распределённая система контроля версий</strong></p>

<div align="center">

[![License](https://img.shields.io/badge/license-SySL%201.0-blue)](../../LICENSE)
[![GitHub](https://img.shields.io/badge/github-celestia--island%2Fnoa-blue.svg)](https://github.com/celestia-island/noa)

</div>

<div align="center">

[English](../en/intro.md) ·
[简体中文](../zhs/intro.md) ·
[繁體中文](../zht/intro.md) ·
[日本語](../ja/intro.md) ·
[한국어](../ko/intro.md) ·
[Français](../fr/intro.md) ·
[Español](../es/intro.md) ·
**Русский** ·
[العربية](../ar/intro.md)

</div>

Изоляция рабочего пространства для каждого агента, журналы JSONL только для добавления (append-only), история на основе снимков и полная совместимость с протоколом git.

## Документация

Архитектура, дизайн и руководства находятся на [docs.celestia.world/en/noa](https://github.com/celestia-island/docs.celestia.world/tree/master/docs/en).

Исходный код: [noa](https://github.com/celestia-island/noa).
