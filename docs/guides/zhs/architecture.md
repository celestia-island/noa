...
```
.nao/
├── noa.redb           # redb 数据库（blobs, trees, snapshots, workspaces, refs）
├── agent-logs/        # 每个工作区的 JSONL 文件
│   └── <ws>.log
├── HEAD               # 当前工作区名称
├── ORIG_HEAD          # 上一个工作区名称
└── config             # TOML 配置
...
```
代理写入 → AgentLog (JSONL, O_APPEND)
                ↓
       SnapshotEngine.compute()
                ↓
    从 write/delete/rename 操作构建 tree
                ↓
    存储 tree → ObjectStore
                ↓
    创建 Snapshot → SnapshotStore
                ↓
    更新工作区 head → WorkspaceManager
```
