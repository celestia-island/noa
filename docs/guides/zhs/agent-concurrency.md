...
```
100 个代理 → 单写入锁队列 → fsync（约 1ms）
1000 次写入/秒 × 1ms = 100% 锁竞争
```
...
```
Agent-001 → agent-logs/001.log  (独立 fd, 0.05ms)
Agent-002 → agent-logs/002.log  (独立 fd, 0.05ms)
...
Agent-100 → agent-logs/100.log  (独立 fd, 0.05ms)
