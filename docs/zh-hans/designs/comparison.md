...
sequenceDiagram
    ...
    A1->>SVN: commit → OK
    A2->>SVN: commit → 等待锁 → OK
    A3->>SVN: commit → 等待锁 → OK
```
...
flowchart LR
    ...
    L1 --> C["Consolidator 按时间戳<br/>合并（后台）"]
    L2 --> C
    L3 --> C
```
...
flowchart LR
    ...
    subgraph Noa
        N1["Write"] --> N2["AgentLog<br/>(追加式，无锁)"] --> N3["Snapshot"] --> N4["Workspace"]
    end
```
...
理由：AI 代理不需要看到冲突标记——它们可以针对最新状态重新生成更改。"upstream-wins" 策略确保向前推进。
...
**noa**：内容寻址 blob，无增量压缩。快照使用 msgpack 编码。权衡：更简单的实现，更快的写入，更大的存储空间。这是可接受的，因为：
- AI 代理产物通常可以重新生成（旧版本是短暂的）
- 存储成本低；代理吞吐量才是昂贵的
- MinIO/S3 后端处理去重
...
noa 不是 Git 的替代品——它是 AI 代理工作流的**补充**。两者并用：
1. AI 代理在 noa 工作区中工作
2. 审核通过的更改通过 push 合并到 Git
3. 人类开发者像以前一样继续使用 Git
