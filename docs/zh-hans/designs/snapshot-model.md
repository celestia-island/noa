...
graph TD
    empty["noa_empty（哨兵）"]
    empty --> a["noa_abc123<br/>(工作区: default, '初始化')"]
    empty --> merge["noa_mno345<br/>(合并 feature-1 和 feature-2 到 default)<br/>parents: [noa_abc123, noa_ghi789, noa_jkl012]"]

    a --> b["noa_def456<br/>(工作区: feature-1, '添加登录')"]
    a --> c["noa_jkl012<br/>(工作区: feature-2, '修复 bug')"]

    b --> d["noa_ghi789<br/>(工作区: feature-1, '添加测试')"]
```
...
```mermaid
flowchart TD
    A["1. AgentLog 重放"] --> A1["读取工作区所有 write/delete/rename 操作"]
    A1 --> B["2. Tree 构建"]
    B --> B1["从父快照的 tree 开始"]
    B1 --> B2["按顺序应用操作"]
    B2 --> B3["存储结果 tree → ObjectStore"]
    B3 --> C["3. 快照创建"]
    C --> C1["构建 Snapshot 结构，包含 tree_hash"]
    C1 --> C2["从内容计算 ID"]
    C2 --> C3["存储到 SnapshotStore（redb 表）"]
    C3 --> D["4. 工作区更新"]
