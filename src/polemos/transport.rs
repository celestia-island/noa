...
impl JsonRpcMessage {
    ...
    pub fn from_frame(data: &[u8]) -> Result<Self> {
        if data.len() < 4 {
            return Err(NoaError::Polemos("frame too short".to_string()));
        }
        let len = u32::from_be_bytes([data[0], data[1], data[2], data[3]]) as usize;
        if data.len() < 4 + len {
            return Err(NoaError::Polemos("frame incomplete".to_string()));
        }
...
impl UnixSocketTransport {
    ...
    pub async fn connect(&self) -> Result<tokio::net::UnixStream> {
        tokio::net::UnixStream::connect(&self.socket_path)
            .await
            .map_err(|e| NoaError::Polemos(format!("unix socket connect failed: {}", e)))
    }
}
