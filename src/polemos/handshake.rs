...
fn create_git_branch(workspace_root: &Path, name: &str, base: &str) -> Result<()> {
    ...
    let output = std::process::Command::new("git")
        ...
        .current_dir(workspace_root)
        .output()
        .map_err(|e| NoaError::Polemos(format!("git checkout -b failed: {}", e)))?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(NoaError::Polemos(format!(
...
fn checkout_git_branch(workspace_root: &Path, name: &str) -> Result<()> {
    let output = std::process::Command::new("git")
        ...
        .current_dir(workspace_root)
        .output()
        .map_err(|e| NoaError::Polemos(format!("git checkout failed: {}", e)))?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(NoaError::Polemos(format!("git checkout failed: {}", stderr)));
    }
